class HomeController < ApplicationController
  def index
  	...
  	if hiker_signed_in?
      @incoming_requests = Request.all.where(dinner_id: Dinner.all.where(hiker: current_hiker)).order('created_at').reverse_order
      @outgoing_requests = Request.all.where(hiker: current_hiker).order('created_at').reverse_order
